...
(function () {
  ...
  /* [data-rows] belongs here. The 2026-09-03 stylesheet stages a list's rows
     from opacity 0 under [data-rows] .row, and this selector did not watch
     that attribute, so the record's five evenings, the six partner
     institutions, the evening's facts and the footer's columns were all in
     the DOM at opacity 0 waiting for an .is-in that never arrived. Three
     sections read as empty on a 1440 screen because of one missing token. */
  const revealables = $$('[data-reveal],[data-plate],[data-rule],[data-rows],.hero');
